...
read_number (const void *arrp, grub_size_t size)
{
  const grub_unaligned_uint16_t *arr = (const grub_unaligned_uint16_t *)arrp;
  long long ret = 0;
  while (size--)
    {
      grub_uint16_t tmp16 = grub_get_unaligned16 (arr++);
      ret = (ret << 16) | grub_le_to_cpu16 (tmp16);
    }
  return ret;
}
