...
static grub_err_t
incr_decr(operation op, int argc, char *argv[])
{
  const char *old;
  char *new;
  long value;
...
grub_cmd_incr(struct grub_command *cmd UNUSED,
              int argc, char *argv[])
{
  return incr_decr(INCREMENT, argc, argv);
}
...
grub_cmd_decr(struct grub_command *cmd UNUSED,
              int argc, char *argv[])
{
  return incr_decr(DECREMENT, argc, argv);
}
