                 ...
                 int argc, char *argv[])
{
  ...
  initrd_mem = grub_efi_allocate_any_pages (BYTES_TO_PAGES(size));
  if (!initrd_mem)
    {
		...
		int argc, char *argv[])
{
  ...
  linux_cmdline = grub_efi_allocate_any_pages (BYTES_TO_PAGES(lh->cmdline_size + 1));
  if (!linux_cmdline)
    {
  ...
  if (!kernel_mem)
    kernel_mem = grub_efi_allocate_any_pages (BYTES_TO_PAGES(lh->init_size));
  if (!kernel_mem)
    {
...
static grub_command_t cmd_linuxefi, cmd_linux, cmd_initrdefi, cmd_initrd;
