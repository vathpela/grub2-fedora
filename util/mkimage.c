...
static inline struct grub_pe32_section_table *
			     ...
			     const char *rsrc_path)
{
  ...
  switch (image_target->id)
	...
	grub_util_info ("copying core.img to FVA 0x%x (0x%lx bytes)",
			raw_data, core_size);
	memcpy (pe_img + raw_data, core_img, core_size);

	/* The magic.  */
	grub_util_info ("copying MSDOS magic to FVA 0x%x (0x%x bytes)",
			0, GRUB_PE32_MSDOS_STUB_SIZE);
	memcpy (header, stub, GRUB_PE32_MSDOS_STUB_SIZE);
	grub_util_info ("copying PEmagic to FVA 0x%x (0x%x bytes)",
			GRUB_PE32_MSDOS_STUB_SIZE, GRUB_PE32_SIGNATURE_SIZE);
	memcpy (header + GRUB_PE32_MSDOS_STUB_SIZE, "PE\0\0", GRUB_PE32_SIGNATURE_SIZE);

	/* The COFF file header.  */
	grub_util_info ("building COFF header at 0x%x (0x%zx bytes)",
			GRUB_PE32_MSDOS_STUB_SIZE + GRUB_PE32_SIGNATURE_SIZE,
			sizeof(struct grub_pe32_coff_header));
	...
	grub_util_info ("building PE32%s header at 0x%lx (0x%zx bytes)",
			o64 ? "+" : "",
			(o64 ? (unsigned long)o64 : (unsigned long)o32) - (unsigned long)header,
			o64 ? sizeof(*o64) : sizeof(*o32));
	...
	scn_size = layout.kernel_size - layout.exec_size;
	PE_OHDR(o32, o64, data_size) = grub_host_to_target32 (scn_size + ALIGN_UP(total_module_size, GRUB_PE32_FILE_ALIGNMENT));
	...
	/*
	 * should we really be doing this if we're not adding the section?
	 */
	PE_OHDR(o32, o64, bss_size) = grub_host_to_target32 (layout.bss_size);
	vma += layout.bss_size;
#if 0
	section = init_pe_section (image_target, section, ".bss",
				   layout.bss_size, &vma,
				   image_target->section_align,
				   0, &raw_data,
				   GRUB_PE32_SCN_MEM_READ
	...
	section = init_pe_section (image_target, section, ".reloc",
				   &vma, scn_size, image_target->section_align,
				   &raw_data, scn_size,
				   GRUB_PE32_SCN_CNT_INITIALIZED_DATA
				   | GRUB_PE32_SCN_MEM_DISCARDABLE
